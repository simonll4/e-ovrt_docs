...
> ⚠️ **2026-08-10 — DOCUMENTO HISTÓRICO.** Auditó las decisiones **antes** de que se
> formalizaran en ADRs. **Los huecos que detectó se cerraron**: el estado de cada decisión
> vive en `../../decisiones/` y en `../../decisiones/estado-de-implementacion-adrs.md`. Se
> conserva porque documenta los contraargumentos que se consideraron. Ver el `README.md` de
> esta carpeta.
...
| H4 | **Fuente viva para EBE sin definir** (OAK-D no disponible). **✎ Update 2026-07-13: cambia de naturaleza — hay DOS fuentes vivas reales (RTSP ya ejercida + OAK-D integrada y verificada E2E)**; el RTSP sintético queda solo para la comparación DBE↔EBE con fuente idéntica. | RTSP sintético: `mediamtx` + `ffmpeg -stream_loop` sirviendo los clips del clip bench como stream en vivo, y/o webcam USB. Tabla 55 de Etapa 3 admite explícitamente "archivo simulado como stream". Cero hardware nuevo; el mismo clip sirve para DBE (archivo) y EBE (stream) → comparación DBE-vs-EBE con fuente idéntica, que es un resultado extra gratis. |
