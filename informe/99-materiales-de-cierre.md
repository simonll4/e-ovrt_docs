...
- **Qué es esto:** los cinco materiales que se pueden cerrar **sin depender del GT del
  lote de internet**, armados y verificados contra artefacto. Es el andamio del capítulo:
  inventario de figuras/tablas, anexo de reproducibilidad, licencias y citas,
  limitaciones + ADRs, y el catálogo de mecanismos.
- **Qué NO es:** **no es redacción de §17.x.** La orden vigente es explícita: la
  redacción arranca después de los runs que falten del lote de internet y de los videos
  V1–V3. Acá no hay prosa de capítulo; hay inventarios, tablas de procedencia y texto
  declarativo corto (licencias, limitaciones) que no cambia con lo que falta.
- **Se lee junto a:** `informe/97` (reglas de redacción y cifras canónicas),
...
**Columna «insumo»** — verificada contra disco el 2026-08-05:
...
|---|---|---|---|---|
| T-68 | **Campañas de Nivel B sobre el banco** (T1, T2, G1, D1, H1, B1: R/P/F1, `t_alert`, TTFD, SDR, FP neg) | `results/clip_bench/index.md` + `*/metrics.json` | R-12/R-13 | ✅ en disco |
...
| T-78 | **Composición del banco de clips** (34 clips, 35 episodios, 34 evaluables, escenarios, negativos) | `datasets/processed/clip_bench/manifest.yaml` + `meta/*.clip.yaml` | R-12 | ✅ en disco |
...
los 4 clips negativos **no** entran a P/R/F1 · se reporta **por estrato y por escenario**,
...
### 2.1 Huellas de los artefactos congelados (verificadas el 2026-08-05)
...
| `manifest.yaml` (banco de clips) | `cef5082e1eb1981c…260e8` | `sha256sum` directo |
...
Corrido el 2026-08-05: **`✅ Todo verificado`** — 8 F1 citados contra `metrics.json`,
3 deltas de bootstrap, 4 enlaces, 31 docs de procedencia (ninguno faltante).

**Lo que este verificador NO cubre, y hay que chequear a mano:** las cifras de **D1, H1,
T2 y B1** (solo cubre T1, G1 y R1–R6). Se verificaron a mano el 2026-08-05 y coinciden.
Si se agrega una campaña al índice, **extender el verificador** o el hueco crece.
...
**Cadena de GT de video (cuando lleguen las anotaciones de CVAT)**
```bash
# el export de CVAT es a nivel PROYECTO: sin este primer paso el GT sale negativo EN SILENCIO
...
| **Lote de internet (14 clips)** | pendiente de GT; levanta L4 | ✎ **2026-08-05: origen registrado** — canal de YouTube **`@HospitalConstruction`** (https://www.youtube.com/@HospitalConstruction). **Es *Standard YouTube License*, no Creative Commons** ⇒ base de uso: **académico/evaluativo con cita y sin redistribución** (postura `chv`), **nunca presentado como licencia de libre uso** | **citar el canal como fuente de las escenas** + los caveats de §3.3 (no es cámara-nativo · caras difuminadas en figuras · velocidad real verificada) |
...
| **L6** | **El tracker no está medido en obra real con multitud** — G1 se verificó en vivo con pocos sujetos; el `track_id` es post-hoc/decorador | declarada y **etiquetada 2026-08-05** | `operacion/98` §6 |
...
| 005 | Distribución de alertas: recorte, canal MQTT, repo propio | **no implementada** (spec 45): se declara como no hecho |
...
## 5. Catálogo de mecanismos: lo que no se mueve cuando llegue el CVAT

Esta es la parte del capítulo que da credibilidad, y es **inmune al lote de internet**: el
GT nuevo puede mover un agregado, no un mecanismo medido sobre las corridas que ya
existen. Texto fuente: los índices de `results/` (verificados mecánicamente) — de ahí se
transcribe, no de acá.
...
- **F-96.7** — 0 FP en negativos en las ocho campañas: con 4 clips es **control, no tasa**.
...
**Lo que sigue estando fuera de este documento, por orden explícita:** la redacción de
§17.x y la regeneración del `informe-project-kit`. Van después de los runs del lote de
internet y de los videos V1–V3.
