...
> ✎ **RUTAS ACTUALIZADAS 2026-08-10 (reorganización de `informe/`).** La carpeta
> `docs/informe/` se partió en dos: **`informe/entregable/`** (el `.docx` y su texto
> extraído: 90, 96a–96e) e **`informe/ajustes/`** (el mapa por etapa + `material-etapa-3/`
> con 91–94 y `gobierno/` con 95, 97, 98, 99). **Los números no cambiaron** — se sigue
> citando "el doc 93", "informe/99". Las rutas de esta tabla ya están corregidas.
> **Al regenerar el `informe-project-kit/`, los nombres aplanados no cambian** (el prefijo
> es la carpeta raíz: `informe-93-...`), pero **hay que sumar los 6 documentos nuevos de
> `informe/ajustes/`**.
...
| **`informe/ajustes/00-mapa-de-ajustes.md`** + los 6 por etapa (`01`…`06`) + **`07-critica-extension-y-poda.md`** | ✎ **AGREGADOS 2026-08-10/11.** **El mapa de ajustes al informe, ordenado de Etapa 1 a Etapa 6** (las seis del Gantt del §14.3): qué hay que corregir en §15/§16, §17.1 y §17.3, y qué hay que **escribir desde cero** en §17.4, §17.5 y §17.6 (las tres vacías). El **`07`** es la **crítica de extensión**: 18 podas medidas (`PODA-nn`, ~27% del texto) que se aplican en el mismo pase que los `AJ-`/`R-`. Reemplaza el relevamiento disperso entre `93`, `nucleo/08`, `sintesis` §7 y `99`. **Es el orden de trabajo del capítulo.** |
| `informe/ajustes/gobierno/97-brief-de-redaccion.md` | Las reglas de redacción (registro, fuentes, honestidad). ⚠️ **Su §5 quedó SUPERSEDIDA** (banco de 34, FAR/hora, lote sin GT): tiene banner de cabecera con la tabla de qué cambió. **Las cifras salen de `results/`, no de §5.** |
...
| **`informe/ajustes/gobierno/99-materiales-de-cierre.md`** | ✎ Inventario de figuras/tablas con su artefacto, anexo de reproducibilidad, licencias y citas obligatorias, limitaciones L1–L8, ADRs y el catálogo de mecanismos. |
| `informe/ajustes/material-etapa-3/92-anexo-concrecion-tecnica.md` | Contratos, APIs y rutas:línea. ⚠ **Ya NO es fuente de cifras** (derogado, doc 97 §2.1): para números, los índices de `results/`. |
| **`informe/ajustes/material-etapa-3/92b-concrecion-distribucion-alertas.md`** | ✎ **AGREGADO 2026-08-10.** El **módulo de distribución de alertas** completo — es el material de **§17.3.10**. Estado declarado en su encabezado: **diseñado y especificado, implementación pendiente**; al redactar, describir el diseño y **declarar el estado**, nunca presentarlo como capacidad verificada. ✎ 2026-08-10: el estatuto lo fija **ADR-016** — trabajo comprometido, no exclusión cerrada (E-06 sigue excluida); el ciclo de vida completo de la alerta está consolidado en **`nucleo/19`**. |
| `informe/ajustes/material-etapa-3/93-redlines-etapa3.md` | El tablero de trabajo: los **26** redlines (R-01…R-26) con casillas. |
| `informe/ajustes/material-etapa-3/94-secciones-nuevas-etapa3.md` | Texto ya redactado en registro de informe (modelo de estilo). |
| `informe/ajustes/material-etapa-3/91-relevamiento-etapa3-vs-implementacion.md` | Qué contradice/falta en el capítulo, con plan A–D. |
| `informe/ajustes/gobierno/95-auditoria-y-plan-de-cierre.md` | Plan de cierre, orden de sacrificio, reglas anti-error. |
| `informe/entregable/90-etapa3-texto-extraido.md` | La Etapa 3 **vigente** (texto a corregir). |
...
| Archivo | Rol |
|---|---|
| `nucleo/historicos/02-revision-critica-etapa3-y-norte.md` | El norte, R1–R4, plan de 12 semanas. |
| `nucleo/historicos/03-spec-plataforma-dos-caminos.md` | Documento rector de las dimensiones D1–D6. |
| `nucleo/historicos/04-diseno-comparativo-estrategias-edir-eind.md` | Pre-registro del experimento D1. |
| `nucleo/historicos/08-alineacion-consolidacion-metodologica.md` | Cómo el set se alinea con la §17.1. |
| `nucleo/historicos/09-justificacion-ovd-y-defensa.md` | Los 5 argumentos, Q&A hostil, cierre del informe. |
| `nucleo/historicos/12-diseno-prompts-y-fusion-ehyb.md` | Prompt sets, comparabilidad, fusión E-HYB, pista doble. |
...
| `informe/entregable/96e-informe-v11-cierre-anexos-referencias.md` | Anexos y referencias del documento — **y los tres placeholders vacíos de §17.4/§17.5/§17.6**. |

### Nivel 3 — opcionales (subir solo si el capítulo lo requiere)

| Archivo | Cuándo |
|---|---|
| `informe/entregable/96c-informe-v11-estado-del-arte.md` (21k palabras) | Solo al retocar §15 o citar modelos — es decir, al trabajar los ajustes `AJ-1.x`. |
| `informe/entregable/96d-informe-v11-marco-teorico.md` (32k palabras) | Solo al retocar §16 o alinear terminología teórica (`AJ-1.16`). |
...
| `nucleo/14`–`19` | ✎ 2026-08-10: **los relevamientos vigentes por servicio** (mapa de la cadena, setup, datasets, media, control, ciclo de vida de la alerta) — relevados contra código, sin cifras. Para "qué es y cómo funciona" cada pieza, entrar por acá. |
| `nucleo/historicos/01/05/06/07/11`, `contingencia/20` | Consulta puntual; ✎ 2026-08-10: movidos a `nucleo/historicos/` (reemplazados por la serie 14–19; manda su banner). `operacion/97` + `informe/92` los resumen. |
