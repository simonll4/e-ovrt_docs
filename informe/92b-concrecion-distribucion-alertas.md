...
- **Estado de implementación:** **diseñado y especificado; implementación pendiente.**
  ...
  Coherente con `ADR-015` y `nucleo/10` (E-06), que registran la distribución como no
  ejercida. **Al completarse la implementación, esta línea es lo único que cambia.**
- **Normativa que lo gobierna** (todos de la **serie del proyecto**, `ADR-001…015` —
  no confundir con la serie interna del control-plane, `ADR-0001…0013` de 4 dígitos):
  ADR-005 (recorte, canal MQTT, repo propio), ADR-011 (frontera de la política: el motor
  ...
  recortada), `nucleo/06` (anexo de diseño completo, E-06).
