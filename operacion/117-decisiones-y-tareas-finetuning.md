...
| **D-FT-15** | Márgenes go/no-go T2 | ✎ **APROBADA, usuario 2026-08-17 — firmada ANTES de todo resultado T2. ✎ APLICADA 2026-08-21: NO-GO** (doc 127) | detalle en §3 (sección D-FT-15). Gain y retención in-domain idénticos a D-FT-12; retención OV nueva sobre COCO val2017 (D-FT-04, **base congelada mAP50 0,434676 ⇒ umbral NO-GO 0,391208**); latencia pareada ≤5 %; expectativa pre-registrada declarada (NO-GO probable, el valor es la curva de capacidad). **Resultado 08-21: gain PASA (`bare_head` +0,0909) · retención in-domain FALLA ×4 (person −49,7 %) · retención OV FALLA (0,1247, −71,3 %) — las 3 expectativas confirmadas** | protocolo T2 y T-FT-064 |
| **D-FT-16** | Corrección del presupuesto de entrenamiento T2 (pre-resultado) | ✎ **aprobada, usuario 2026-08-18 — ANTES de toda evaluación tuned** | detalle en §3. `optimizer=auto` resultó ciego al conteo de parámetros entrenables (LR idéntico para T1 y T2 pese a ×3.343 de diferencia); job `1167864` submuestreado. **Re-correr 60 épocas DESDE EL PESO BASE** con optimizador EXPLÍCITO (SGD, lr0=0,01, momentum=0,937) + patience=15, todo fijado antes de ver resultado alguno. **No** continuar desde el `best.pt` de `1167864`: la evidencia muestra que es peor punto de partida que el peso base (T2 ep1 0,0490 vs T1 ep1 0,1507, con caída a 0,0255 en ep2). **✎ Resultado (08-20/21): el job `1167982` aplicó el optimizador corregido pero colapsó igual (early stop 16/60, best=ep1) — el hallazgo pasó de "LR mal calculado" a "full FT inviable a este volumen de datos" (doc 127 §1)** | T-FT-064 (re-corrida v2) |
...
| **T-FT-060** Diseñar/ejecutar T2 | ✎ **done (2026-08-21) — tier COMPLETO, veredicto NO-GO (doc 127)** | D-FT-14 ✓, D-FT-15 ✓, D-FT-16 ✓ | `experimental-setup`, remoto | tier exploratorio; desglose en T-FT-061…066, todas cerradas |
...
| **T-FT-064** RUN T2 full + eval única bench | ✎ **done (2026-08-21)** | T-FT-063 ✓, D-FT-16 ✓ | remoto, local | el job `1167864` (10 épocas, `optimizer=auto`) completó 08-18 pero quedó **submuestreado** (LR ciego al conteo de params ⇒ D-FT-16) y NO se evaluó; la re-corrida v2 **job `1167982`** (SGD lr0=0,01 desde peso base, techo 60/patience 15) corrió 08-20: **colapsó — early stop 16/60, `best_epoch=1`**. Brazo bench one-shot consumido 08-21 con ese `best.pt` (fork firmado por el usuario): 6.477/6.477, 0 errores. **mAP50 0,2374; `bare_head` 0,0909 (sólo shel5k)**. Doc 127 §3 |
| **T-FT-065** Retención OV + latencia pareada | ✎ **done (2026-08-21) — latencia NO medida, con causa** | T-FT-064 ✓ | local | promoción previa con regla 7 completa (`t2_promotion_1167982.json`); COCO tuned 5.000/5.000: **mAP50 0,1247 vs base 0,4347 (−71,3 %)**. Latencia pareada no medida: dos gates de retención ya fallan y no puede revertir el veredicto (criterio F-123.1); queda opcional como dato descriptivo. Doc 127 §3.4 y §4 |
| **T-FT-066** Go/no-go T2 y constancia | ✎ **done (2026-08-21) — NO-GO** | T-FT-065 ✓ | `docs` | D-FT-15 aplicada sin renegociar: gain PASA, las dos retenciones FALLAN. Las 3 expectativas pre-registradas confirmadas. **F-127.1: el fallo de T1 no era capacidad — es estructural (datos)**; curva de 3 puntos completa. Manifiesto `t2_go_no_go_1167982.json`; constancia y propagación en doc 127 |
