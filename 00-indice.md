...
| 54 | `operacion/54-video-gt-lab-y-contrato-gt.md` | **video-gt-lab ejecutado (2026-07-11) — el tooling del spec 43.** Pipeline semi-automático de GT temporal de video en 5 etapas: `prepare_clip.sh` (CFR) → `preannotate_video` (GDINO-**base**, anti-circularidad, + ByteTrack) → corrección en CVAT (atributos radio `unknown/true/false` — la incertidumbre NUNCA fabrica infracción) → `derive_clip_gt.py` (episodios determinísticos + `provenance`) → `validate_clip_gt.py` + kappa doble anotación. Smoke E2E real en GPU sobre obra real (41 tracks). La auditoría posterior (3 agentes) descubrió que **`evaluate-alerts` v2 rechazaba el GT real** (identidad `source_id`/`subject_key`) — reparado: convención **`source_id = clip_id`**, ventanas caller > `provenance.pattern_set_ms` > defaults (`effective_matching_windows`). Suites 88/503/199, **SIN COMMITEAR (3 repos)**. Deja PENDIENTE la ejecución del banco (§5): roundtrip CVAT, grabación A+C, **SDR+TTFD (gap de código, no de datos)**, promoción al banco, `clip_id` en manifiesto spec 44. |
