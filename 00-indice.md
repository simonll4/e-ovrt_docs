...
| 57 | `operacion/57-validacion-metodologica-externa-duracion-clips.md` | **Validación metodológica externa de la duración de clips y la evaluación por episodio (2026-07-18/19), a partir de dos preguntas del director** (¿un clip de 12 s perjudica la estrategia de patrones? ¿30 s es buena duración?). Contrasta el diseño contra la única práctica externa comparable (**i-LIDS** UK Home Office, **TRECVID SED**, THUMOS/ActivityNet, ODAS, literatura PPE): valida el matching por episodio+ventana, `re_alerts` como mejora sobre el defecto documentado de i-LIDS, y la persistencia en segundos; y detecta 4 gaps accionables (G1 falta FAR/hora con clips soak, G2 percentiles no cierran con el n del banco, G3 faltan confusables semánticos, G4 el bug del matching greedy pasa a bloqueante en P8). Deriva la **duración ideal bimodal** (~15 s ausencia / ~25–30 s alerta medida), la fórmula de dimensionamiento (`pre-roll + evento + cola`, censura temporal), plantillas de clip listas para grabar, y el chequeo automático `dimensioning_warning` a agregar al validador de `clip_gt.v2`. **Explica por qué `video16_clip10` (episodio en t=0) da TTFD=0 como artefacto de recorte, no medición.** Corrige la duración de grabación de doc 55 PASO 4 / spec 43 §3. |
